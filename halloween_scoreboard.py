		...
		for w in range(1,2):
			...
			for team in df.index:
				...
				if r == 490:
					...
					tmp = r-210
					df.loc[team,f'WODdisplay{w}'] = f'{r} reps: {tmp} thrusters (50/35)'
				elif r<= 280:
					tmp = r-240
					df.loc[team,f'WODdisplay{w}'] = f'{r} reps: {tmp} CTB pull-ups'
				elif r<= 300:
					tmp = r-280
					df.loc[team,f'WODdisplay{w}'] = f'{r} reps: {tmp} thrusters (60/42.5)'
				elif r<= 320:
					tmp = r-300
					df.loc[team,f'WODdisplay{w}'] = f'{r} reps: {tmp} bar muscle-ups'
				elif r<= 330:
					tmp = r-320
					df.loc[team,f'WODdisplay{w}'] = f'{r} reps: {tmp} thrusters (70/50)'
				elif r<= 340:
					tmp = r-330
					df.loc[team,f'WODdisplay{w}'] = f'{r} reps: {tmp} rope-climbs'
				elif r<490:
					tmp = r-340
